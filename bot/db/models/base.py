"""
Base model classes for the database layer.

This module provides the foundation for all database models, implementing common
functionality such as CRUD operations, validation, serialization, and relationships.
"""

import inspect
import json
import logging
import sqlite3
from datetime import datetime
from typing import Any, ClassVar, Dict, List, Optional, Set, Tuple, Type, TypeVar, Union, cast

from bot.db.connection import Database, QueryError, TransactionError, get_db

# Set up logger for this module
logger = logging.getLogger(__name__)

# Type variable for type hinting
T = TypeVar('T', bound='BaseModel')


class ModelError(Exception):
    """Base exception for model-related errors."""
    pass


class ValidationError(ModelError):
    """Exception raised for validation errors."""
    pass


class NotFoundError(ModelError):
    """Exception raised when a model instance is not found."""
    pass


class DuplicateError(ModelError):
    """Exception raised when a unique constraint is violated."""
    pass


class RelationshipError(ModelError):
    """Exception raised for relationship-related errors."""
    pass


class Field:
    """
    Field definition for model attributes.

    This class provides metadata for model fields, including type information,
    validation rules, and relationship details.
    """

    def __init__(
        self,
        field_type: Type,
        primary_key: bool = False,
        unique: bool = False,
        nullable: bool = True,
        default: Any = None,
        foreign_key: Optional[str] = None,
        validators: Optional[List[callable]] = None
    ):
        """
        Initialise a field definition.

        Args:
            field_type (Type): Field data type.
            primary_key (bool): Whether this field is a primary key.
            unique (bool): Whether this field has a unique constraint.
            nullable (bool): Whether this field can be NULL.
            default (Any): Default value for the field.
            foreign_key (Optional[str]): Foreign key reference (table.column).
            validators (Optional[List[callable]]): List of validator functions.
        """
        self.field_type = field_type
        self.primary_key = primary_key
        self.unique = unique
        self.nullable = nullable
        self.default = default
        self.foreign_key = foreign_key
        self.validators = validators or []

    def validate(self, value: Any) -> None:
        """
        Validate a value against this field's constraints.

        Args:
            value (Any): Value to validate.

        Raises:
            ValidationError: If the value does not meet the field's constraints.
        """
        # Check for NULL
        if value is None:
            if not self.nullable:
                raise ValidationError(f"Field cannot be NULL")
            return

        # Check type
        if not isinstance(value, self.field_type) and value is not None:
            # Special handling for bool fields (SQLite stores as integers)
            if self.field_type is bool and isinstance(value, int):
                pass
            else:
                raise ValidationError(
                    f"Expected type {self.field_type.__name__}, got {type(value).__name__}")

        # Run custom validators
        for validator in self.validators:
            try:
                if not validator(value):
                    raise ValidationError(
                        f"Validation failed with validator {validator.__name__}")
            except Exception as e:
                if isinstance(e, ValidationError):
                    raise
                raise ValidationError(str(e)) from e


class Relationship:
    """
    Definition of a relationship between models.

    This class provides metadata for model relationships, such as
    one-to-many, many-to-one, and many-to-many relationships.
    """

    def __init__(
        self,
        related_model: str,
        relation_type: str,
        foreign_key: str,
        back_populates: Optional[str] = None
    ):
        """
        Initialise a relationship definition.

        Args:
            related_model (str): Name of the related model class.
            relation_type (str): Type of relationship ('one_to_many', 'many_to_one', 'many_to_many').
            foreign_key (str): Foreign key field name.
            back_populates (Optional[str]): Name of the attribute on the related model
                that contains the back-reference to this relationship.
        """
        self.related_model = related_model
        self.relation_type = relation_type
        self.foreign_key = foreign_key
        self.back_populates = back_populates


class ModelMeta(type):
    """
    Metaclass for model classes.

    This metaclass processes field and relationship definitions and sets up
    the necessary metadata for model classes.
    """

    def __new__(mcs, name, bases, attrs):
        """
        Create a new model class.

        Args:
            name (str): Class name.
            bases (tuple): Base classes.
            attrs (dict): Class attributes.

        Returns:
            type: New model class.
        """
        if name == 'BaseModel':
            # Don't process the base class itself
            return super().__new__(mcs, name, bases, attrs)

        # Get or create __fields__ and __relationships__ dictionaries
        fields = {}
        relationships = {}

        # Get table name from class or generate from class name
        table_name = attrs.get('__tablename__')
        if table_name is None:
            # Convert CamelCase to snake_case for table name
            import re
            s1 = re.sub('(.)([A-Z][a-z]+)', r'\1_\2', name)
            table_name = re.sub('([a-z0-9])([A-Z])', r'\1_\2', s1).lower()
            attrs['__tablename__'] = table_name

        # Process fields
        for attr_name, attr_value in list(attrs.items()):
            if isinstance(attr_value, Field):
                fields[attr_name] = attr_value
                # Remove the field from the class attributes
                del attrs[attr_name]
            elif isinstance(attr_value, Relationship):
                relationships[attr_name] = attr_value
                # Remove the relationship from the class attributes
                del attrs[attr_name]

        # Store fields and relationships in class attributes
        attrs['__fields__'] = fields
        attrs['__relationships__'] = relationships

        # Create new class
        return super().__new__(mcs, name, bases, attrs)


class BaseModel(metaclass=ModelMeta):
    """
    Base class for all database models.

    This class provides common functionality for working with database records,
    including CRUD operations, validation, and relationship handling.
    """

    # Class variables for metadata
    __tablename__: ClassVar[str] = ''
    __fields__: ClassVar[Dict[str, Field]] = {}
    __relationships__: ClassVar[Dict[str, Relationship]] = {}

    # Instance variables
    id: Optional[int] = None
    _db: Optional[Database] = None
    _values: Dict[str, Any] = {}
    _loaded_relationships: Dict[str, Any] = {}
    _is_new: bool = True
    _is_deleted: bool = False

    def __init__(self, **kwargs):
        """
        Initialise a model instance.

        Args:
            **kwargs: Field values.
        """
        # Initialise internal state
        self._values = {}
        self._loaded_relationships = {}
        self._is_new = True
        self._is_deleted = False

        # Set field values from kwargs
        for field_name, value in kwargs.items():
            if field_name in self.__fields__:
                self._values[field_name] = value
            elif field_name == 'id':
                self.id = value
                self._is_new = False  # If ID is provided, assume existing record
            else:
                # Ignore fields that don't exist in the model
                pass

        # Apply default values for missing fields
        for field_name, field in self.__fields__.items():
            if field_name not in self._values and field.default is not None:
                if callable(field.default):
                    self._values[field_name] = field.default()
                else:
                    self._values[field_name] = field.default

    def __getattr__(self, name: str) -> Any:
        """
        Get a field or relationship value.

        Args:
            name (str): Field or relationship name.

        Returns:
            Any: Field or relationship value.

        Raises:
            AttributeError: If the field or relationship does not exist.
        """
        if name in self._values:
            return self._values[name]
        elif name in self.__fields__:
            return None
        elif name in self.__relationships__:
            # Load the relationship if not already loaded
            if name not in self._loaded_relationships:
                self._load_relationship(name)
            return self._loaded_relationships[name]
        else:
            raise AttributeError(
                f"'{self.__class__.__name__}' has no attribute '{name}'")

    def __setattr__(self, name: str, value: Any) -> None:
        """
        Set a field or relationship value.

        Args:
            name (str): Field or relationship name.
            value (Any): Field or relationship value.

        Raises:
            AttributeError: If the field or relationship does not exist.
            ValidationError: If the value does not meet the field's constraints.
        """
        if name.startswith('_'):
            # Allow setting internal attributes directly
            super().__setattr__(name, value)
        elif name == 'id':
            # Set ID directly
            super().__setattr__(name, value)
            if value is not None:
                self._is_new = False
        elif name in self.__fields__:
            # Validate the value before setting it
            field = self.__fields__[name]
            field.validate(value)
            self._values[name] = value
        elif name in self.__relationships__:
            # Set the relationship value
            self._loaded_relationships[name] = value
        else:
            # Default behaviour for other attributes
            super().__setattr__(name, value)

    def get_db(self) -> Database:
        """
        Get the database connection.

        Returns:
            Database: Database connection.
        """
        if self._db is None:
            self._db = get_db()
        return self._db

    def to_dict(self) -> Dict[str, Any]:
        """
        Convert the model instance to a dictionary.

        Returns:
            Dict[str, Any]: Dictionary representation of the model.
        """
        result = {'id': self.id}
        result.update(self._values)
        return result

    def from_dict(self, data: Dict[str, Any]) -> 'BaseModel':
        """
        Update the model instance from a dictionary.

        Args:
            data (Dict[str, Any]): Dictionary containing field values.

        Returns:
            BaseModel: Updated model instance (self).

        Raises:
            ValidationError: If a value does not meet field constraints.
        """
        for field_name, value in data.items():
            if field_name == 'id':
                self.id = value
                self._is_new = False
            elif field_name in self.__fields__:
                # Validate value before setting
                field = self.__fields__[field_name]
                field.validate(value)
                self._values[field_name] = value

        return self

    def from_row(self, row: sqlite3.Row) -> 'BaseModel':
        """
        Update the model instance from a database row.

        Args:
            row (sqlite3.Row): Database row.

        Returns:
            BaseModel: Updated model instance (self).
        """
        if row is None:
            raise NotFoundError("Record not found")

        # Convert row to dict
        data = dict(row)

        # Update instance from dict
        return self.from_dict(data)

    def validate(self) -> None:
        """
        Validate all fields.

        Raises:
            ValidationError: If any field value does not meet its constraints.
        """
        for field_name, field in self.__fields__.items():
            value = self._values.get(field_name)
            field.validate(value)

    def save(self) -> 'BaseModel':
        """
        Save the model instance to the database.

        This method creates a new record if the instance is new,
        or updates an existing record if the instance has an ID.

        Returns:
            BaseModel: Saved model instance (self).

        Raises:
            ValidationError: If validation fails.
            QueryError: If the database operation fails.
        """
        # Validate all fields
        self.validate()

        # Get database connection
        db = self.get_db()

        try:
            if self._is_new:
                # Insert new record
                data = {k: v for k, v in self._values.items() if v is not None}
                self.id = db.insert(self.__tablename__, data)
                self._is_new = False
                logger.debug(
                    f"Created {self.__class__.__name__} with ID {self.id}")
            else:
                # Update existing record
                data = {k: v for k, v in self._values.items() if v is not None}
                db.update(self.__tablename__, data, "id = ?", (self.id,))
                logger.debug(
                    f"Updated {self.__class__.__name__} with ID {self.id}")

            return self
        except QueryError as e:
            # Check for unique constraint violation
            if "UNIQUE constraint failed" in str(e):
                raise DuplicateError(
                    f"Record already exists with these unique fields") from e
            raise

    def delete(self) -> None:
        """
        Delete the model instance from the database.

        Raises:
            NotFoundError: If the instance has no ID.
            QueryError: If the database operation fails.
        """
        if self.id is None:
            raise NotFoundError("Cannot delete a record without an ID")

        if self._is_deleted:
            logger.warning(
                f"{self.__class__.__name__} with ID {self.id} already deleted")
            return

        # Get database connection
        db = self.get_db()

        # Delete the record
        db.delete(self.__tablename__, "id = ?", (self.id,))
        self._is_deleted = True
        logger.debug(f"Deleted {self.__class__.__name__} with ID {self.id}")

    def refresh(self) -> 'BaseModel':
        """
        Reload the model instance from the database.

        Returns:
            BaseModel: Refreshed model instance (self).

        Raises:
            NotFoundError: If the instance has no ID or the record is not found.
            QueryError: If the database operation fails.
        """
        if self.id is None:
            raise NotFoundError("Cannot refresh a record without an ID")

        if self._is_deleted:
            raise NotFoundError(
                f"{self.__class__.__name__} with ID {self.id} has been deleted")

        # Get database connection
        db = self.get_db()

        # Get the record
        query = f"SELECT * FROM {self.__tablename__} WHERE id = ?"
        row = db.fetchone(query, (self.id,))

        if row is None:
            raise NotFoundError(
                f"{self.__class__.__name__} with ID {self.id} not found")

        # Update instance from row
        self.from_row(row)
        self._loaded_relationships.clear()  # Clear cached relationships
        return self

    def _load_relationship(self, relationship_name: str) -> None:
        """
        Load a relationship.

        Args:
            relationship_name (str): Relationship attribute name.

        Raises:
            RelationshipError: If the relationship cannot be loaded.
        """
        if self.id is None:
            # Can't load relationships for a record without an ID
            self._loaded_relationships[relationship_name] = None
            return

        relationship = self.__relationships__[relationship_name]
        related_model_name = relationship.related_model
        relation_type = relationship.relation_type
        foreign_key = relationship.foreign_key

        try:
            # Import the related model class
            module_parts = self.__module__.split('.')
            # Try different module paths to find the related model
            module_paths = [
                '.'.join(module_parts[:-1]) + '.' + related_model_name.lower(),
                self.__module__,
                'bot.db.models.' + related_model_name.lower()
            ]

            related_model_class = None
            for module_path in module_paths:
                try:
                    module = __import__(module_path, fromlist=[
                                        related_model_name])
                    if hasattr(module, related_model_name):
                        related_model_class = getattr(
                            module, related_model_name)
                        break
                except ImportError:
                    continue

            if related_model_class is None:
                raise RelationshipError(
                    f"Could not find related model class '{related_model_name}'")

            # Load the relationship based on its type
            if relation_type == 'many_to_one':
                # Get the single related record
                foreign_id = self._values.get(foreign_key)
                if foreign_id is None:
                    self._loaded_relationships[relationship_name] = None
                else:
                    related_instance = related_model_class.get_by_id(
                        foreign_id)
                    self._loaded_relationships[relationship_name] = related_instance
            elif relation_type == 'one_to_many':
                # Get all related records
                related_instances = related_model_class.find_by(
                    foreign_key, self.id)
                self._loaded_relationships[relationship_name] = related_instances
            elif relation_type == 'many_to_many':
                # Many-to-many relationships require a junction table
                # This would require additional implementation
                raise NotImplementedError(
                    "Many-to-many relationships are not implemented yet")
            else:
                raise RelationshipError(
                    f"Unknown relationship type: {relation_type}")
        except Exception as e:
            if isinstance(e, (RelationshipError, NotImplementedError)):
                raise
            raise RelationshipError(
                f"Error loading relationship '{relationship_name}': {str(e)}") from e

    @classmethod
    def get_by_id(cls: Type[T], id: int) -> T:
        """
        Get a model instance by ID.

        Args:
            id (int): Record ID.

        Returns:
            T: Model instance.

        Raises:
            NotFoundError: If the record is not found.
            QueryError: If the database operation fails.
        """
        db = get_db()
        query = f"SELECT * FROM {cls.__tablename__} WHERE id = ?"
        row = db.fetchone(query, (id,))

        if row is None:
            raise NotFoundError(f"{cls.__name__} with ID {id} not found")

        # Create instance and populate from row
        instance = cls()
        instance.from_row(row)
        instance._is_new = False
        return instance

    @classmethod
    def get_by(cls: Type[T], field: str, value: Any) -> T:
        """
        Get a model instance by a field value.

        Args:
            field (str): Field name.
            value (Any): Field value.

        Returns:
            T: Model instance.

        Raises:
            NotFoundError: If the record is not found.
            QueryError: If the database operation fails.
        """
        db = get_db()
        query = f"SELECT * FROM {cls.__tablename__} WHERE {field} = ?"
        row = db.fetchone(query, (value,))

        if row is None:
            raise NotFoundError(
                f"{cls.__name__} with {field}={value} not found")

        # Create instance and populate from row
        instance = cls()
        instance.from_row(row)
        instance._is_new = False
        return instance

    @classmethod
    def find_by(cls: Type[T], field: str, value: Any) -> List[T]:
        """
        Find model instances by a field value.

        Args:
            field (str): Field name.
            value (Any): Field value.

        Returns:
            List[T]: List of model instances.

        Raises:
            QueryError: If the database operation fails.
        """
        db = get_db()
        query = f"SELECT * FROM {cls.__tablename__} WHERE {field} = ?"
        rows = db.fetchall(query, (value,))

        # Create instances from rows
        instances = []
        for row in rows:
            instance = cls()
            instance.from_row(row)
            instance._is_new = False
            instances.append(instance)

        return instances

    @classmethod
    def find_all(cls: Type[T]) -> List[T]:
        """
        Get all model instances.

        Returns:
            List[T]: List of all model instances.

        Raises:
            QueryError: If the database operation fails.
        """
        db = get_db()
        query = f"SELECT * FROM {cls.__tablename__}"
        rows = db.fetchall(query)

        # Create instances from rows
        instances = []
        for row in rows:
            instance = cls()
            instance.from_row(row)
            instance._is_new = False
            instances.append(instance)

        return instances

    @classmethod
    def count(cls) -> int:
        """
        Get the count of records.

        Returns:
            int: Record count.

        Raises:
            QueryError: If the database operation fails.
        """
        db = get_db()
        query = f"SELECT COUNT(*) as count FROM {cls.__tablename__}"
        row = db.fetchone(query)

        if row is None:
            return 0

        return row['count']

    @classmethod
    def exists(cls, id: int) -> bool:
        """
        Check if a record exists.

        Args:
            id (int): Record ID.

        Returns:
            bool: True if the record exists, False otherwise.

        Raises:
            QueryError: If the database operation fails.
        """
        db = get_db()
        query = f"SELECT 1 FROM {cls.__tablename__} WHERE id = ?"
        row = db.fetchone(query, (id,))

        return row is not None

    @classmethod
    def exists_by(cls, field: str, value: Any) -> bool:
        """
        Check if a record exists by a field value.

        Args:
            field (str): Field name.
            value (Any): Field value.

        Returns:
            bool: True if the record exists, False otherwise.

        Raises:
            QueryError: If the database operation fails.
        """
        db = get_db()
        query = f"SELECT 1 FROM {cls.__tablename__} WHERE {field} = ?"
        row = db.fetchone(query, (value,))

        return row is not None

    @classmethod
    def create(cls: Type[T], **kwargs) -> T:
        """
        Create a new model instance and save it to the database.

        Args:
            **kwargs: Field values.

        Returns:
            T: Created model instance.

        Raises:
            ValidationError: If validation fails.
            QueryError: If the database operation fails.
        """
        instance = cls(**kwargs)
        instance.save()
        return instance

    @classmethod
    def bulk_create(cls: Type[T], records: List[Dict[str, Any]]) -> List[T]:
        """
        Create multiple records in a single transaction.

        Args:
            records (List[Dict[str, Any]]): List of record data.

        Returns:
            List[T]: List of created model instances.

        Raises:
            ValidationError: If validation fails.
            QueryError: If the database operation fails.
        """
        db = get_db()
        instances = []

        try:
            with db.transaction():
                for record_data in records:
                    instance = cls(**record_data)
                    instance.save()
                    instances.append(instance)
        except Exception as e:
            logger.error(f"Error in bulk_create: {str(e)}", exc_info=True)
            raise

        return instances

    @classmethod
    def delete_by_id(cls, id: int) -> bool:
        """
        Delete a record by ID.

        Args:
            id (int): Record ID.

        Returns:
            bool: True if the record was deleted, False if it wasn't found.

        Raises:
            QueryError: If the database operation fails.
        """
        db = get_db()
        result = db.delete(cls.__tablename__, "id = ?", (id,))
        return result > 0

    @classmethod
    def delete_by(cls, field: str, value: Any) -> int:
        """
        Delete records by a field value.

        Args:
            field (str): Field name.
            value (Any): Field value.

        Returns:
            int: Number of records deleted.

        Raises:
            QueryError: If the database operation fails.
        """
        db = get_db()
        result = db.delete(cls.__tablename__, f"{field} = ?", (value,))
        return result

    @classmethod
    def query(cls: Type[T], sql: str, params: Optional[Tuple] = None) -> List[T]:
        """
        Execute a custom SQL query and return model instances.

        Args:
            sql (str): SQL query.
            params (Optional[Tuple]): Query parameters.

        Returns:
            List[T]: List of model instances.

        Raises:
            QueryError: If the database operation fails.
        """
        db = get_db()
        rows = db.fetchall(sql, params)

        # Create instances from rows
        instances = []
        for row in rows:
            instance = cls()
            try:
                instance.from_row(row)
                instance._is_new = False
                instances.append(instance)
            except Exception as e:
                logger.warning(
                    f"Error creating instance from query result: {str(e)}", exc_info=True)
                continue

        return instances
